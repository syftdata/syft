 ...
 */
export function unicodeWords(val: string): string[] {
  return val.replace(/['\u2019]/g, '').match(reUnicodeWords) ?? [];
}
...
export function convertCase(val: string, to: NamingCase): string {
  switch (to) {
    case NamingCase.SNAKE:
      return unicodeWords(val)
