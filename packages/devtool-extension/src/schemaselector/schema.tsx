...
const EditableFieldRenderer = ({ field, val, setVal }: { field: EventField, val: string, setVal: (val: string) => void }) => {
  return (
    <Flex.Row
      gap={2}
      className={css`
    ...
    >
      ...
      <Input.L12 type="text" value={val} onChange={(e) => setVal(e.target.value)} />
    </Flex.Row>
  );
};
...
const SchemaPropsRenderer = ({ data, onUpdate }: { 
  ...
  return (
    <Flex.Col gap={4}>
      {data.schema.fields.map((field, index) => (
        event ?         
        <EditableFieldRenderer key={index} 
          field={field} 
