...
export class ActionContext extends BaseAction {
  ...
  constructor(action: Action, scriptType: ScriptType, actionState: ActionState) {
    super()
    this.action = action
    ...
    switch (type) {
      ...
      case ActionType.Input:
        return `Fill ${truncateText(
          ...
          16,
        )} on <${tagName.toLowerCase()}> ${getBestSelectorForAction(this.action, this.scriptType)}`
      ...
      case ActionType.DragAndDrop:
        return `Drag n drop ${getBestSelectorForAction(this.action, this.scriptType)} from (${
          this.action.sourceX
        }, ${this.action.sourceY}) to (${this.action.targetX}, ${this.action.targetY})`
      default:
        return ''
    }
  }
...
export abstract class ScriptBuilder {
  ...
  abstract dragAndDrop: (sourceX: number, sourceY: number, targetX: number, targetY: number) => this
  ...
  private transformActionIntoCodes = (actionContext: ActionContext) => {
    ...
    switch (actionContext.getType()) {
      ...
      case ActionType.Hover:
        ...
        break
      case ActionType.Keydown:
        this.keydown(bestSelector as string, action.key ?? '', causesNavigation)
        break
      case ActionType.Input: {
      ...
      case ActionType.Resize:
        ...
        break
      case ActionType.Wheel:
        this.wheel(action.deltaX, action.deltaY, action.pageXOffset, action.pageYOffset)
        break
      case ActionType.FullScreenshot:
        this.fullScreenshot()
        break
      case ActionType.AwaitText:
        ...
        break
      case ActionType.DragAndDrop:
        this.dragAndDrop(action.sourceX, action.sourceY, action.targetX, action.targetY)
        break
      default:
        break
    }
  ...
  buildCodes = () => {
    ...
    for (const actionContext of this.actionContexts) {
      if (!actionContext.getActionState().isStateful) {
        if (prevActionContext !== undefined && prevActionContext.getActionState().isStateful) {
...
export class PlaywrightScriptBuilder extends ScriptBuilder {
  ...
  wheel = (deltaX: number, deltaY: number) => {
    this.pushCodes(`await page.mouse.wheel(${Math.floor(deltaX)}, ${Math.floor(deltaY)});`)
    return this
  }
  ...
  dragAndDrop = (sourceX: number, sourceY: number, targetX: number, targetY: number) => {
    this.pushCodes(
      [
...
test('Written with Syft Studio', async ({ page }) => {${this.codes.join('')}});`
  }
}
...
export class PuppeteerScriptBuilder extends ScriptBuilder {
  ...
  fill = (selector: string, value: string, causesNavigation: boolean) => {
    const pageType = `page.type('${selector}', ${JSON.stringify(value)})`
    if (causesNavigation) {
      this.pushCodes(this.waitForSelectorAndNavigation(`${selector}:not([disabled])`, pageType))
    } else {
      // Do more actionability checks
      this.pushCodes(
        `await ${this.waitForSelector(`${selector}:not([disabled])`)};\n  await ${pageType};`,
      )
    }
    return this
  }
...
export class CypressScriptBuilder extends ScriptBuilder {
  ...
  wheel = (deltaX: number, deltaY: number, pageXOffset?: number, pageYOffset?: number) => {
    this.pushCodes(`cy.scrollTo(${Math.floor(pageXOffset ?? 0)}, ${Math.floor(pageYOffset ?? 0)});`)
    return this
  }
