...
export function usePreferredLibrary() {
  const [preferredLibrary, setPreferredLibrary] = useState<ScriptType | null>(null)

  useEffect(() => {
    localStorageGet(['preferredLibrary']).then(({ preferredLibrary: storedPreferredLibrary }) => {
...
export function usePreferredBarPosition(defaultPosition: BarPosition) {
  const [preferredBarPosition, setPreferredBarPosition] = useState<BarPosition | null>(
    defaultPosition,
  )
...
export function useRecordingState() {
  ...
  useEffect(() => {
    localStorageGet(['recording', 'recordingTabId']).then(({ recording, recordingTabId }) => {
