...
function ActionListItem({ action, stepNumber }: { action: Action; stepNumber: number }) {
  return (
      ...
      {action.type === ActionType.Click ? (
        <>
          ...
          (action.selectors.text?.length ?? -1) < 75 ? (
            ...
            <span className="mono">{getBestSelectorForAction(action, ScriptType.Playwright)}</span>
          )}
        </>
        ...
        <>
          <span className="em-text">Load</span> <span className="mono">"{action.url}"</span>
        </>
        ...
        <>
          ...
          on <span className="mono">{getBestSelectorForAction(action, ScriptType.Playwright)}</span>
        </>
      ) : action.type === ActionType.Keydown ? (
        <>
          <span className="em-text">Press</span> <span className="mono">"{action.key}"</span> on{' '}
          <span className="mono">{getBestSelectorForAction(action, ScriptType.Playwright)}</span>
        </>
    ...
    <>
      ...
      <Flex.Col className="ActionList">
        {actions
          ...
          .map((action, i) => (
            <ActionListItem key={`${action.type}-${i}`} action={action} stepNumber={i + 1} />
          ))}
      </Flex.Col>
    </>
  )
