...
function isEventFromOverlay(event: Event) {
  return (
    event.composedPath().find((element) => (element as HTMLElement).id === 'overlay-controls') !=
    null
  )
}
...
function buildBaseAction(event: Event, overrideTarget?: HTMLElement): BaseAction {
  const target = overrideTarget ?? (event.target as HTMLElement)

  return {
    isPassword: target instanceof HTMLInputElement && target.type.toLowerCase() === 'password',
...
class Recorder {
  ...
  constructor({ onInitialized, onAction }: { onInitialized?: any; onAction?: any }) {
    ...
    localStorageGet(['recording']).then(({ recording }) => {
      ...
      chrome.storage.onChanged.addListener((changes) => {
        if (changes.recording != null && changes.recording.newValue != changes.recording.oldValue) {
      ...
      if (onInitialized != null) {
        onInitialized(this._recording?.[this._recording.length - 1], this._recording)
      }
    })
  }
  ...
  private onClick = (event: Event) => {
    ...
    const predictedTarget = parentElement?.tagName === 'A' ? parentElement : target
  ...
  private onDrag = (event: DragEvent) => {
    ...
    } else if (event.type === 'drop' && lastAction.type === ActionType.DragAndDrop) {
      this.updateLastRecordedAction({
        targetX: event.x,
        targetY: event.y,
      })
  ...
  private onBackgroundMessage = (request: any) => {
    ...
    if (request != null && request.type === 'onHoverCtxMenu' && this.lastContextMenuEvent != null) {
      const action = {
        ...buildBaseAction(this.lastContextMenuEvent),
        type: 'hover',
        selectors: genSelectors(this.lastContextMenuEvent.target as HTMLElement),
      }
      this.appendToRecording(action)
    }
    if (
